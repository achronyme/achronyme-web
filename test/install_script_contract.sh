...
grep -Fq -- '--version 0.1.1' "$project_root/public/install.sh"
...
test "$("$install_root/bin/ach" --version)" = "ach 0.1.1"
...
grep -qx \
    ...
    "$request_log"
grep -qx \
    "https://github.com/achronyme/achronyme/releases/download/v0.1.1/$artifact.tar.gz" \
    "$request_log"
grep -qx \
    "https://github.com/achronyme/achronyme/releases/download/v0.1.1/$artifact.tar.gz.sha256" \
    "$request_log"
