...
grep -Fq 'releases/0.1.2' "$sidebar"
...
grep -Fq '`--error-format json` selects JSON output' "$project_root/src/content/docs-en/cli/commands.mdx"
grep -Fq '`--error-format json` selecciona salida JSON' "$project_root/src/content/docs-es/cli/commands.mdx"
grep -Fq 'Operational errors stay operational' "$release_en"
grep -Fq 'Los errores operativos siguen siendo errores' "$release_es"
grep -Fq 'One JSON output contract' "$release_en"
grep -Fq 'Un solo contrato de salida JSON' "$release_es"
