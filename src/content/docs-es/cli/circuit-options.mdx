...
Los valores se parsean como elementos del campo primo activo (BN254 por defecto, configurable via `--prime`). Soporta decimal, hex (`0xFF`) y valores negativos. Las dos banderas son mutuamente excluyentes.
...
| `--solidity <ruta>` | Escribir un contrato verificador Solidity en `<ruta>` (solo R1CS/Groth16). La ruta es obligatoria. |
...
ach circuit circuit.ach --inputs "x=42,y=7" --solidity Verifier.sol
