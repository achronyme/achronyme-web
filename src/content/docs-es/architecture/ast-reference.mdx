...
- El pase del resolver anota cada `ExprId` con un `SymbolId` en `SymbolTable`. Ver `resolve/src/annotate.rs`. El resolver **no** muta el AST — construye una tabla lateral indexada por `ExprId`.
...
| Anotación del resolver | `resolve/src/annotate.rs` |
