...
- Archivo: `achronyme-parser/src/ast.rs`
- Cada nodo lleva un `Span`. Re-exportado desde `diagnostics/src/span.rs`.
...
De `achronyme-parser/src/ast.rs:52-141`:
...
De `achronyme-parser/src/ast.rs:188-331`. Cada variante lleva `id: ExprId` y `span: Span` además de los campos listados.
...
| Variante | Campos extra | Notas |
|----------|--------------|-------|
| `Error` | — | Emitido por puntos de sincronización del parser (`synchronize()` en `achronyme-parser/src/parser/core.rs`). |
...
- El pase del resolver anota cada `ExprId` con un `SymbolId` en `SymbolTable`. Ver `resolve/src/annotate/`. El resolver **no** muta el AST — construye una tabla lateral indexada por `ExprId`.
...
| Componente            | Archivo |
|-----------------------|---------|
| Tipos del AST         | `achronyme-parser/src/ast.rs` |
| Span                  | `diagnostics/src/span.rs` |
| Entrada del parser    | `achronyme-parser/src/lib.rs` |
| Anotación del resolver | `resolve/src/annotate/` |
| Tabla de símbolos     | `resolve/src/table.rs` |
