...
Archivo: `diagnostics/src/suggest.rs`. Búsqueda por distancia de Levenshtein contra una lista de identificadores válidos (nombres de builtins, variables en alcance, nombres de plantillas). Umbral: distancia ≤ 2 y >= 50% de similitud.
...
Archivo: `diagnostics/src/renderer.rs`.
...
| Renderer | `diagnostics/src/renderer.rs` |
| `Suggestion` + applicability | `diagnostics/src/suggest.rs` |
| Diagnósticos del parser | `achronyme-parser/src/parser/core.rs` |
| Diagnósticos de Circom | `circom/src/diagnostics.rs` |
| Diagnósticos de IR | `ir/src/diagnostics.rs` |
