...
```rust
ArtikCall { outputs: Vec<Variable>, input_vars: Vec<Variable>,
            program_bytes: Vec<u8> }
```

Despacha a la [VM de testigos Artik](./artik-vm). Se emite cuando el IR carga una `Instruction::WitnessCall` — típicamente de un cuerpo de función Circom que el pase `artik_lift` compiló a bytecode.

En tiempo de generación de testigo, el generador decodifica `program_bytes`, conecta `input_vars` al slice de señales de solo-lectura de Artik, y ejecuta el programa. Cada valor que Artik escribe vía `WriteWitness` aterriza en uno de los cables `outputs`. Ver [VM de Testigos Artik](./artik-vm) para el formato de bytecode y la superficie del lift.

Cuando un cuerpo lifteado coincide con una rutina bignum multi-limb conocida, Artik toma un **camino rápido de intrínseco nativo** (`ModInv`, `ModExp`, `Prod`, `LongDiv`) en vez de interpretarlo limb por limb — reemplazar un inverso modular de Fermat interpretado bajó el recorrido de testigos de `ECDSAVerify(64,4)` de aproximadamente 226 s a 64 s end-to-end sin cambio en el conteo de restricciones ni la verificación. Con este camino, **la generación de testigos de ECDSA secp256k1 ahora funciona end-to-end**: el circuito de referencia de circomlib más pesado se liftea, genera un testigo, y prueba a través de Groth16 con el testigo verificando.
...
| Componente | Archivo |
|-----------|------|
| WitnessOp & WitnessGenerator | `zkc/src/witness/` |
| R1CS compile_ir_with_witness | `zkc/src/r1cs_backend.rs` |
| Plonkish compile_ir_with_witness | `zkc/src/plonkish_backend/` |
| Parámetros de Poseidon | `constraints/src/poseidon/` |
