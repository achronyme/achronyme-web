...
Archivo: `circom/src/lexer.rs`. Escáner de bytes single-pass. Desambiguación maximal-munch para la familia de operadores de señales:
...
Archivo: `circom/tests/adversarial.rs`. Cuatro tests prueban que los deltas de constraints vs circom O2 son ganancias de folding LC, no bugs de bajo-constraint:
...
| Expresiones | `circom/src/lowering/expressions.rs` |
| Sentencias | `circom/src/lowering/statements.rs` |
| Const fold | `circom/src/lowering/const_fold.rs` |
| Lift a Artik | `circom/src/lowering/artik_lift.rs` |
| Context | `circom/src/lowering/context.rs` |
| Diagnósticos | `circom/src/diagnostics.rs` |
| Tests adversariales | `circom/tests/adversarial.rs` |
| Tests E2E | `circom/tests/e2e.rs` |
