...
En `zkc/src/plonkish_backend.rs`, agrega un brazo de match en `compile_ir`:
...
Si la instrucción asigna variables intermedias, registra un `WitnessOp` en `zkc/src/r1cs_witness.rs`.
...
En `akronc/src/codegen.rs`, emite el opcode durante la compilación:
...
- [ ] `zkc/src/plonkish_backend.rs` — emisión de puertas Plonkish
- [ ] `zkc/src/r1cs_witness.rs` — computación de testigo (si hay cables intermedios)
