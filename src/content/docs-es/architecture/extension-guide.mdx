...
En `ir-core/src/types/instruction.rs`, agrega una nueva variante al enum `Instruction`:
...
En `ir/src/lower/`, maneja la nueva instrucción en el método apropiado (ej., `lower_call` para funciones integradas, `lower_expr` para operadores):
...
En `ir/src/eval/`, maneja la instrucción en el evaluador apropiado:
...
En `zkc/src/r1cs_backend.rs`, agrega un brazo de match en `compile_ir`:
...
En `zkc/src/plonkish_backend/`, agrega un brazo de match en `compile_ir`:
...
Si la instrucción asigna variables intermedias, registra un `WitnessOp` en `zkc/src/witness/`.
...
En `ir/src/lower/`, agrega un brazo de match en `lower_call`:
...
Crea un archivo de pase bajo `ir/src/passes/`, por ejemplo `my_pass.rs`:
...
En `akron/src/stdlib/core.rs`, agrega la implementación:
...
En `akron/src/specs.rs`, agrega una entrada a `NATIVE_TABLE`:
...
En `akron/src/machine/native.rs`, agrega el brazo de match en `bootstrap_natives`:
...
En `akron/src/opcode.rs`, agrega una constante:
...
En `akronc/src/codegen/`, emite el opcode durante la compilación:
...
En `akron/src/machine/vm.rs`, agrega un brazo de match en `interpret_inner`:
...
- [ ] `ir-core/src/types/instruction.rs` — enum `Instruction` + `result_var` + `has_side_effects` + `operands`
- [ ] `ir/src/lower/` — emitir instrucción desde AST
- [ ] `ir/src/eval/` — evaluación concreta
...
- [ ] `zkc/src/r1cs_backend.rs` — generación de restricciones R1CS
- [ ] `zkc/src/plonkish_backend/` — emisión de puertas Plonkish
- [ ] `zkc/src/witness/` — computación de testigo (si hay cables intermedios)
