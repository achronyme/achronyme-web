...
### Eliminación a nivel de R1CS (más allá de los pases de IR)

Los tres pases anteriores operan sobre el IR SSA. Más allá de ellos, el backend R1CS ejecuta optimización a **nivel de restricciones** (O1/O2): eliminación lineal y sustitución que pliegan cables redundantes después de la emisión de restricciones. Este es el paso crítico para la solidez — las sustituciones se resuelven con **resolución de ciclos por clúster** (SCC de Tarjan + RREF, incluyendo auto-ciclos) para que ningún cable eliminado sobreviva como referencia colgante en una restricción superviviente (un peligro de testigo falsificable). Los testigos se mantienen bit-idénticos y el conteo optimizado para `ECDSAVerify(64,4)` se sostiene en ~1.49M, por debajo de circom. Ver [Backends](./backends) para los detalles.
...
| Componente | Archivo |
|-----------|------|
| Tipos | `ir-core/src/types/` |
| Bajada | `ir/src/lower/` |
| Evaluador | `ir/src/eval/` |
