---
title: "Akron VM y Bytecode"
description: "Máquina virtual de propósito general basada en registros: opcodes, codificación de valores y formato binario."
---

**Akron** es la VM de bytecode de propósito general de Achronyme — un intérprete basado en registros que ejecuta archivos `.achb` producidos por el compilador de bytecode. Corre el lenguaje Achronyme en tiempo de ejecución (`ach run`) y despacha bloques `prove {}` serializados a un proof handler inyectado en modo VM.

Akron está emparejado con [Artik](./artik-vm), la VM dedicada a cómputo de testigos. La división es intencional: Akron maneja scripting y estado de heap mutable con GC, mientras que Artik maneja la matemática de testigos sin efectos secundarios, sin heap ni GC.
...
Akron {
    ...
    stack: Box<[Value]>,     // pila de registros fija (65,536 slots)
    ...
    prototypes: Vec<u32>,       // handles de funciones para CLOSURE
    open_upvalues: lista enlazada, // variables de pila capturadas
    stress_mode: bool,          // forzar GC en cada ciclo
    prove_handler: Option<Box<dyn ProveHandler>>,
    circom_handler: Option<Box<dyn CircomWitnessHandler>>,
}
```
...
| 13 | `BIGINT` | handle u32 -> arena de bigints |
| 14 | `BYTES` | handle u32 -> arena de bytes (constantes ProveIR) |
| 15 | `CIRCOM_HANDLE` | handle u32 -> arena de circom_handles |
...
Akron y [Artik](./artik-vm) son VMs hermanas con roles disjuntos:
...
| 34 variantes de opcode asignadas, formato de 3 operandos | ~25 opcodes, registros tipados |
| Invocado por `ach run`, modo VM | Despachado desde R1CS witness-gen (`WitnessOp::ArtikCall`) |

Ver [VM de Testigos Artik](./artik-vm) para los detalles del camino de testigos.
...
| Metodos de prototipo | `akron/src/machine/prototype.rs` y `akron/src/stdlib/` |
| Codificacion de valores | `memory/src/value.rs` |
| Struct Function | `memory/src/heap/objects.rs` |
