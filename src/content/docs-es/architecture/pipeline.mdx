...
import { compilePipelineSvgEs } from '../../../lib/diagrams/compile-pipeline';
...
<AchDiagram svg={compilePipelineSvgEs} ariaLabel="Pipeline de compilación de Achronyme" />
...
**Punto de entrada:** `akronc::Compiler::compile()`

Para `ach run`, el AST se compila a bytecode para la VM basada en registros (34 variantes de opcode asignadas). Un optimizador de bytecode de 3 pases se ejecuta después de la compilación:
...
| D | Instanciar | ProveIR + valores capturados del scope → SSA IR (Bytecode-Oriented: liftado a un programa de [Lysis VM](/docs/architecture/lysis-vm/) cuya ejecución emite SSA) |
...
Compila archivos `.circom` a través del mismo pipeline ProveIR → IR → Backend. El frontend Circom agrega seguridad estructural: un witness hint `<--` / `-->` que no queda fijado por una constraint correspondiente se reporta como E100. Las asignaciones dobles se reportan por separado como warnings W103, así que la documentación no debe tratar W103 como una constraint.
...
| Resolución de símbolos | `resolve/src/annotate/` |
| Lexer Circom | `circom/src/lexer/` |
| Parser Circom | `circom/src/parser/` |
| Análisis Circom | `circom/src/analysis/` |
| Lowering Circom | `circom/src/lowering/` |
| Compilador de Bytecode | `akronc/src/codegen/` |
| Optimizador de Bytecode | `akronc/src/optimizer/` |
| Compilador ProveIR | `ir-forge/src/ast_lower/api.rs` |
| Instanciación ProveIR | `ir-forge/src/instantiate/` |
| Lift Lysis | `ir-forge/src/lysis_lift/` |
...
| Generación de Testigos | `zkc/src/witness/` |
