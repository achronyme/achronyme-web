...
Usa `--circuit-stats` con `ach run`, `ach circuit` o `ach circom` para
inspeccionar costos. Achronyme reporta dos totales deliberadamente distintos:

- `PRE-OPTIMIZATION ESTIMATE` es el estimado estático derivado de las
  categorías del IR optimizado antes de finalizar R1CS.
- `FINAL PROVING CONSTRAINTS` es el conteo exacto después de optimizar R1CS.
  Este sistema es el que recibe Groth16 y el que queda ligado a trusted keys.

Usa el conteo final para capacidad de proving, ceremonia y evidencia de
release. El estimado sirve para atribución, pero no identifica un artefacto
criptográfico.
