...
`ach run` genera pruebas solo despues de elegir una fuente explicita de llave.
Usa setup de una sola parte para desarrollo local:

```bash
ach --insecure-dev-setup run my_program.ach
```

Cada bloque `prove {}` compila su circuito, verifica el witness y devuelve una
prueba criptografica. Las llaves de desarrollo se cachean para ejecuciones
posteriores, pero no son confiables para produccion.

Para BN254 Groth16 de produccion, selecciona un store derivado de ceremonia cuyo
manifest coincida con el circuito optimizado exacto:

```bash
ach --trusted-key-dir ./trusted-keys run my_program.ach
```

Sin una de las dos opciones, generar falla de forma cerrada. Consulta
[Generacion de Pruebas y Confianza](/es/docs/zk-concepts/proof-generation/)
antes de usar el resultado fuera de desarrollo local.
...
```bash
ach --insecure-dev-setup run my_program.ach --prove-backend plonkish
```

La generacion Plonkish sigue siendo solo para desarrollo en la linea 0.1.0.
