...
La seccion `[circom]` en `achronyme.toml` declara las rutas de busqueda de librerias por proyecto:
...
`ach run`, `ach circuit` y `ach circom` consumen `[circom].libs` automaticamente, igualando la ergonomia de `[project].entry` y los otros defaults manejados por el manifest. Las banderas CLI `-l/--lib` se suman a esa lista en lugar de reemplazarla. Ve [Limitaciones y Roadmap](/docs/circom/limitations/) para trabajo de seguimiento en esta area.
