...
El modo VM se introdujo con el frontend de circom e intencionalmente es mas angosto en alcance que el [Modo Circuito](/docs/circom/circuit-mode/).
...
Los template arguments siguen la misma regla que el modo circuito, con una restriccion adicional del modo VM:

- **Deben ser constantes en tiempo de compilacion** (igual que modo circuito).
- **Deben ser literales enteros** en el callsite.
...
Cosas que aun no estan soportadas:

- **Constant folding para template arguments** — `let N = 8; Num2Bits(N)(x)` en modo VM.
- **Signal inputs de array** — llamar un template que toma `signal input in[n]`. Esta es una restriccion solo del modo VM atada al layout de registros del opcode `CallCircomTemplate`, no un limite de la evaluacion del testigo.
...
Estos son limites de alcance del *dispatch* del modo VM. El evaluador de testigo subyacente es completamente general — la generacion de testigo en modo circuito ya maneja parametros de template de array y rutinas bignum multi-limb (por ejemplo, los testigos de ECDSA secp256k1 corren a traves de la VM Artik). Todo esto esta trackeado en [Limitaciones y Roadmap](/docs/circom/limitations/). Ninguno de estos afecta el dispatch del modo circuito, que permanece como el path principal soportado.
