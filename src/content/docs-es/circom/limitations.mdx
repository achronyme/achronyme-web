...
| `EdDSAPoseidonVerifier` | — | — | ✓ Compile + instantiate verificados (31,685 nodos IR → 22,271 instrucciones tras optimización); el test R1CS pasa con `enabled=0` |
| `ECDSAVerify(64, 4)` / ECDSA secp256k1 | — | — | ✓ Compila + genera testigo + prueba end-to-end (Groth16); ~1.49M constraints R1CS optimizadas, por debajo de circom para el mismo circuito; el testigo verifica |
| Mux, gates, bitify, comparadores | — | — | ✓ Usados transitivamente por los anteriores |

`ECDSAVerify(64, 4)` es el circuito de referencia mas pesado de circomlib, y su testigo depende de aritmetica multi-limb de secp256k1 — inverso y exponenciacion modular, producto escolar y division larga. Achronyme ahora genera ese testigo de forma nativa a traves de intrinsecos de la VM Artik, en lugar de un path interpretado de inverso modular de Fermat, asi que el circuito completo compila, genera testigo y prueba sin intervencion manual.
...
### Estructurales — por diseno (modo circuito)

Estas restricciones vienen del modelo de lowering de **modo circuito** de Achronyme (IR totalmente desenrollado, R1CS primero), no de trabajo faltante. Aplican a la generacion de constraints; la computacion del testigo se maneja por separado en la VM Artik, que es mas permisiva (ve la nota abajo):
...
La restriccion de unrolling es especifica del IR de constraints R1CS/Plonkish. **Las funciones de testigo elevadas a la VM Artik si soportan loops con limites de runtime y acceso con indice variable** — asi es exactamente como funciona la generacion de testigo de ECDSA secp256k1, donde las rutinas bignum multi-limb (inverso modular, division larga) corren con limites de loop e indices de array que solo se conocen en runtime. El sistema de constraints permanece totalmente desenrollado; el lado del testigo no necesita estarlo.
...
El modo VM (llamar templates desde codigo no-`prove` bajo `ach run`) es mas angosto que el modo circuito. Restricciones actuales:
...
### Tutoriales y guias de migracion (en progreso)
