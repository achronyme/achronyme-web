...
Accede a los valores exportados usando el operador de ruta `::`: `alias::nombre`. La resolución ocurre completamente en tiempo de compilación — no existe un objeto `alias` en runtime, y el global mangled `alias::nombre` se despacha por la misma tabla que los statics nativos como `Int::MAX` o `Field::ZERO`.

:::note
Las versiones anteriores usaban notación de punto (`alias.nombre`). Esa forma ahora produce un error de migración que apunta al spelling correcto `alias::nombre`. El cambio se hizo para que las llamadas calificadas por módulo resuelvan de forma idéntica dentro de código VM y dentro de bloques `prove {}`, donde el dispatch en HashMap del esquema antiguo no estaba disponible.
:::
...
print(math::mul(3, 4))   // namespace — con `::`
```
...
Los imports de namespace y selectivos conservan el contexto del módulo que
define la función dentro de bloques `prove`. Los captures de arrays tipados
pueden cruzar hacia una función de prueba importada sin colisionar con IDs de
expresiones del archivo raíz.
...
| `alias::name` | Acceder a un binding de namespace (resuelto en tiempo de compilación) |
