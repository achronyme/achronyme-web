...
Cuando un diagnóstico nace en un módulo, cada formato de salida conserva el
archivo importado canónico, el span exacto y el texto fuente correspondiente.
Un error en `lib/proofs.ach` nunca se renderiza contra la línea o el fragmento
del programa raíz.
...
Esta advertencia solo aplica a imports selectivos (`import { ... } from`). Los imports de namespace (`import "..." as alias`) no se verifican — cada acceso pasa por el operador de ruta `::` (`alias::nombre`), así que el uso se rastrea por miembro a través del lookup de dispatch en tiempo de compilación en lugar de un flag por-alias.
