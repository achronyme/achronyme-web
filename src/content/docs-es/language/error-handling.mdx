...
| `ProveHandlerNotConfigured` | Se alcanzó un `prove {}` pero no se inyectó ningún manejador de pruebas en la VM. Esto solo ocurre en integraciones personalizadas que ejecutan la VM directamente — `ach run` siempre conecta el manejador Groth16 nativo, por lo que el CLI nunca lo dispara. |
