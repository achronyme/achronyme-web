...
### Expandido: cobertura E2E R1CS de circomlib — 13 circuitos verificados

La verificación completa del sistema de restricciones (compilar → instanciar → R1CS → testigo → verificar) ahora cubre 13 circuitos reales de circomlib:

| Circuito | Restricciones | Patrón |
|---|---|---|
| Num2Bits(8) | 17 | Descomposición de bits |
| IsZero | 4 | Gadget inverso |
| LessThan(8) | ~20 | Composición de componentes |
| Poseidon(2) | ~361 | Permutación + MDS |
| MiMCSponge(2,220,1) | ~3,000 | Feistel, 220 rondas |
| BabyJubjub (Add+Dbl+Check) | 48 | Curva Edwards |
| EscalarMulFix(3) | — | Montgomery + Mux3 |
| EscalarMulAny(254) | 10,224 | Scalar mul completo (Groth16 verificado) |
| **EdDSAPoseidon** | **>10,000** | **Verificación de firma completa** |
| **Switcher** | — | Swap condicional |
| **Mux3 / Mux4** | — | Selector 8-way / 16-way |
| **AliasCheck** | 201,267 | Detección de alias del módulo del campo |
| **Sign** | 1,181 | Signo de elemento de campo |
| **Pedersen(8)** | **394** | **Hash EC (Window4 + Montgomery)** |

EdDSaPoseidon (el "boss final" — verificación completa de firma EdDSA sobre BabyJubjub con hash Poseidon) y Pedersen (hash basado en curva elíptica, construcción completamente diferente a los hashes algebraicos) representan los dos circuitos más complejos de circomlib verificados exitosamente.

### Nuevo: optimizador de restricciones lineales R1CS

Optimizador R1CS multi-paso que reduce el conteo de restricciones después de la compilación:

1. **Eliminación lineal**: sustituye wires definidos por restricciones lineales (`k * LC = LC`), eliminando la restricción por completo
2. **Heurística de frecuencia**: selecciona el wire más referenciado para sustitución, maximizando simplificaciones en cascada
...
EdDSaPoseidon: 8,514 → 7,752 restricciones (0.96× vs circom `--O1` con 8,086).
