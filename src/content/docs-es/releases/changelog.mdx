...
Todos los cambios relevantes de Achronyme se documentan aquí. Cada release actualiza todos los crates del workspace a la misma versión.

## 0.1.2 - parche de verificacion separada

> Publicado el 2026-08-09 desde la revision del core
> `cd7a6e66e133bebd8e2026e321a4c85023c311f7`.

**Achronyme 0.1.2 corrige la salida de verificacion separada sin cambiar el
lenguaje, bytecode, ABI del runtime, formatos de pruebas, formato de trusted
keys ni contratos del LSP.**

- Los artefactos ausentes, ilegibles o que no pueden inspeccionarse reportan su
  causa operativa en vez del resultado enganoso `proof invalid`.
- La bandera global `--error-format json` selecciona salida estructurada para
  verificacion separada salvo que un `verify --format` explicito la reemplace.
- Los registros legacy de estrategia y auditoria se eliminaron del arbol
  actual sin reescribir commits ni tags historicos.

---

## 0.1.1 - parche de madurez

> Publicado el 2026-08-08 desde la revision del core
> `b1774e88671a1889146804cb812cb099eb9cc006`.

**Achronyme 0.1.1 endurece los contratos introducidos en 0.1.0 sin cambiar el
bytecode, ABI del runtime, formato de trusted keys ni politica de proving.**

### Correctitud y observabilidad

- Los diagnosticos de modulos importados conservan archivo canonico, span
  exacto y source correspondiente en salida humana, JSON y corta.
- Imports de namespace y selectivos conservan el contexto del modulo que los
  define durante la compilacion de pruebas, incluidos captures de arrays
  tipados.
- Las estadisticas separan el estimado previo a optimizar R1CS del conteo final
  exacto que usa proving.
- `max_tasks` cuenta cada tarea hija viva; `max_task_scopes` cuenta todos los
  scopes estructurados vivos simultaneamente en el runtime.

### Fixture de madurez end-to-end

- Tilino Lab es una subasta privada modular que cubre modulos, arrays tipados,
  concurrencia estructurada, canales acotados, recursos TCP y de archivo con
  propietario, capacidades explicitas, Poseidon, Merkle y una prueba Groth16.
- Su contrato comprueba paridad interprete/JIT, verificacion separada, rechazo
  de inputs publicos alterados, autoridad denegada, agotamiento de recursos,
  estadisticas R1CS exactas y el limite AOT standalone.

---

## 0.1.0 - release estable

> Publicado el 2026-08-08 desde la revision del core
> `fd07b38e16256e2ed6a8f2b438d340a681c9b0ac` despues de completar el gate de
> ceremonia independiente, beacon publico e interoperabilidad bidireccional.

**Achronyme 0.1.0 agrega concurrencia estructurada, autoridad explicita del
host y un limite de confianza de proving que falla de forma cerrada.**

### Concurrencia estructurada y autoridad del host

- Los scopes lexicos `concurrent` agregan `spawn` ligado al scope, `await`
  explicito, carreras de tareas, cancelacion cooperativa, canales acotados y
  diagnosticos de tareas.
- La inferencia de efectos y los manifiestos de capacidades separan la
  ejecucion del host del codigo `prove` y `circuit`.
- Los recursos de archivos y TCP tienen propiedad explicita, ejecucion acotada
  y limpieza determinista. El filesystem y la red requieren grants exactos.
- El interprete, LLVM JIT, el runtime AOT y la matriz de soporte WASM definen la
  misma semantica de tareas sin otorgar acceso ambiental al host.

### Confianza de proving

- La generacion de pruebas rechaza el setup local implicito. Las pruebas de
  desarrollo requieren un opt-in inseguro y explicito.
- El proving BN254 Groth16 de produccion puede cargar un trusted-key store
  inmutable ligado al R1CS optimizado exacto y al transcript de ceremonia.
- El CLI permite verificacion Groth16 separada para BN254 y BLS12-381.
- La generacion de pruebas BLS12-381 Groth16 y Plonkish sigue siendo solo para
  desarrollo en 0.1.0.
- El playground web centraliza su politica de setup de desarrollo y etiqueta
  sus artefactos como no confiables para produccion.

### Compatibilidad y verificacion

- `concurrent`, `spawn` y `await` ahora son palabras reservadas. El runtime aun
  carga bytecode 0.0.1 y 0.1.0 emite el nuevo formato de tareas estructuradas.
- El CI de release cubre todo el workspace, interoperabilidad de proving y los
  gates LLVM JIT y AOT en Linux x86_64 y aarch64.
- La ceremonia de produccion registra la contribucion independiente de fase 2,
  un beacon publico comprometido con anterioridad e interoperabilidad
  bidireccional con snarkjs.

---

## 0.0.1 - 2026-08-02

**Achronyme inicia la numeracion `MAJOR.MINOR.PATCH` sin sufijos para su linea
pre-1.0.** Este reinicio intencional sigue a la serie historica
`0.1.0-beta.22`. El lenguaje y el API todavia pueden cambiar entre versiones
menores `0.x`.

### Ejecucion nativa y empaquetado

- El JIT ORC de LLVM 21 forma parte del CLI por defecto, con seleccion explicita
  del interprete y fallback visible cuando LLVM no esta disponible.
- `ach aot` genera ejecutables nativos y descubre el runtime AOT empaquetado de
  Achronyme sin variables de entorno manuales.
- Los releases de Linux incluyen bundles reproducibles, licencias, checksums
  SHA-256 y binarios independientes para instaladores existentes.
- El instalador web verifica el bundle de Linux e instala `ach` y el runtime
  AOT.
- `ach-server` fija todas las dependencias del compilador y proving a la misma
  revision fuente `0.0.1`.

### Verificacion

- El gate de produccion pasa en runners Linux x86_64 y aarch64 nativos.
- El gate cubre JIT, fallback al interprete, AOT, limites de recursos, I/O real
  de archivos, ejecucion desde el layout instalado y carga diferida de LLVM.
- macOS y Windows siguen siendo targets compatibles con el interprete; no estan
  certificados por el gate nativo de Linux.

---

## 0.1.0-beta.22 — 2026-06-12

**secp256k1 `ECDSAVerify(64, 4)` — el circuito de referencia más pesado de circomlib — ahora compila, genera testigo y prueba de extremo a extremo.** El R1CS optimizado llega a ~1.49M de constraints, por debajo del conteo del propio circom para el mismo circuito; el testigo verifica y se produce y comprueba una prueba Groth16.

### Nuevo: intrínsecos bignum nativos en Artik

La generación de testigo de ECDSA antes dependía de un camino interpretado de inverso modular de Fermat. La VM Artik ahora incluye intrínsecos bignum nativos — `ModInv`, `ModExp`, `Prod` y `LongDiv` — que reemplazan el loop interno interpretado. El walk de testigo de `ECDSAVerify(64, 4)` baja de ~226s a ~64s de extremo a extremo, sin cambios en el conteo de constraints ni en el resultado de verificación.

### Nuevo: VM Artik con múltiples subprogramas

El builder de Artik ahora emite programas con múltiples subprogramas con `Call`/`Return` entre frames, slices de fila 2D, loops descendentes y con cota en runtime, parámetros de array y operaciones bignum con precisión de campo. Se añadió una comparación ordenada sin signo a nivel de campo (`FCmpLt`) para que las comparaciones con y sin signo se queden a precisión completa de campo. Nuevos opcodes desde los docs anteriores: `Call`, `FIDiv`, `FIRem`, `FShr`, `FAnd`, `FPow2`, `FCmpLt`, `ArrayId`, `ArrayFromId`.

### Seguridad: fix de soundness en la sustitución R1CS

El camino greedy de sustitución R1CS podía dejar wires eliminados aún referenciados tras la optimización — un peligro de testigo falsificable. Esto se cierra por completo vía resolución de ciclos por cluster: los testigos quedan byte-idénticos y el conteo de constraints optimizado se mantiene por debajo de circom. Es un fix de correctitud relevante a seguridad, no un cambio de rendimiento.

### Rendimiento

- **Fusión de instantiate**, **replay de slots del entorno de testigo** y un **optimizador fusionado sobre eventos del interner** recortan trabajo redundante por instancia en el camino lean de prove.
- **Amortización del eje de compilación** más un allocator **jemalloc** reducen la fragmentación y los walks repetidos.
- La memoria residente pico en circuitos a escala ECDSA se reduce aproximadamente a la mitad.

### Nuevo: cobertura de benchmarks

El corpus de benchmarks añade el circuito principal de **Semaphore** y más templates de circomlib junto a la referencia de ECDSA.

### Herramientas

- El CI ahora corre en un **runner self-hosted con toolchain pineado**, eliminando el drift de clippy entre releases.
...
- `ECDSAVerify(64, 4)` compila, genera un testigo que verifica y produce una prueba Groth16 que verifica en BN254.
- R1CS optimizado ~1.49M de constraints, por debajo de circom para el mismo circuito.
- Gate de tests del workspace 4,400+ pasando a través de 19 crates.
- Salida de testigo byte-idéntica antes y después del fix de soundness en la sustitución.

---

## 0.1.0-beta.21 — 2026-05-05

**SHA-256 desde circomlib ahora compila de extremo a extremo a través de bloques `.ach prove` para todo `N` en `Sha256(N)`.** Un release de fixes enfocado que cierra dos bugs distintos entre el playground y los templates pesados de circomlib.

### Corregido: la instanciación de templates de librería pasa por lowering que preserva el For

Los bloques `.ach prove` que importaban un template de circomlib (`Sha256`, `Poseidon`, etc.) antes pasaban el cuerpo del template a ProveIR con todos los loops totalmente desenrollados. Para `Sha256(64)` eso era un cuerpo plano de ~280 K instrucciones, forzando a Lysis a asignar el frame entero de una sola vez y tropezar con el techo de 255 slots.

`instantiate_template_into` ahora pasa por la misma primitiva `lower_library_template` que usa el camino standalone de `ach circom`. Los loops quedan enrollados, Lysis liftea iteraciones uniformes en templates reusables, y el presupuesto de frame por template se queda bajo el cap.

### Corregido: SsaVars definidos en el scope exterior preservados en splits intermedios

Aun con loops For preservados, el walker per-iter que materializa cuerpos de loop anchos tenía un predicado de live-set que solo conservaba variables *referenciadas dentro del cuerpo del loop*. Un `Const(Field(0))` enlazado en el scope exterior del ProveIR sintético (usado para cablear entradas constantes) nunca era referenciado dentro del loop de rondas, así que el primer split intermedio lo filtraba fuera, nunca lo desplazaba al heap, y desaparecía de ambos `ssa_to_reg` y `ssa_to_heap`. El primer `AssertEq(?, %0)` exterior tras el loop fallaba entonces con `walker: undefined SsaVar %0`.

El predicado ganó una tercera cláusula — `!body_defined.contains(v)` — para que cualquier var en `ssa_to_reg` que no se haya definido dentro del cuerpo del loop persista por la partición cold, se desplace al heap, y se recupere vía `LoadHeap` desde cualquier template subsecuente.

`Sha256(64)` tuvo suerte con el predicado viejo: el orden de los splits enrutaba `%0` por cold antes de que el predicado lo descartara. `Sha256(N)` con `N` en {8, 16, 32} no tuvo esa suerte — ahora compilan.

### Nuevo: entry points del LSP WASM para `.circom`

El LSP WASM del editor en el navegador expone diagnósticos de `.circom` con la misma superficie que el binario nativo: E100-E102 (soundness de constraints), W101-W103 (advertencias de signals no verificados), E300-E306 (parser).

### Verificación

- 7/7 probes de dispatch SHA-256 pasan Lysis
- 5/5 regresiones cli SHA-256 vía `.ach prove`, incluyendo el `sha256_8_compiles` antes pineado con `#[ignore]` y una nueva variante con entradas mixtas
- 4068/4068 unit tests + 2/2 doc tests + 182/182 tests de integración
- `cross_path_prove_baseline` byte-idéntico (el fix es solo asignación interna de frame)
- Ratio end-to-end de `perf_external` SHA-256(64) 0.96× vs circom+snarkjs

---

## 0.1.0-beta.20 — 2026-05-03

**Frontend Circom, Lysis (tercera VM), descomposición del workspace en 19 crates, optimizador R1CS iterativo y SHA-256(64) end-to-end. Compila archivos `.circom` a través del pipeline de Achronyme con garantías de seguridad estructural que el compilador de referencia no provee.**
...
Poseidon(2) compila la implementación real de circomlib a través de cadenas de includes (`poseidon.circom` → `poseidon_constants.circom` → funciones helper) usando evaluación de arrays en tiempo de compilación para las 960+ constantes de ronda.
...
| Circuito | Nuestro O1 | circom | Patrón |
|---|---|---|---|
| Num2Bits(8) | **9** | 17 | Descomposición de bits |
| IsZero | 4 | 4 | Gadget inverso |
| LessThan(8) | **10** | 20 | Composición de componentes |
| Poseidon(2) | **240** | 243 | Permutación + MDS |
| MiMCSponge(2,220,1) | **1,317** | 1,320 | Feistel, 220 rondas |
| BabyJubjub (Add+Dbl+Check) | **30** | 48 | Curva Edwards |
| EscalarMulFix(253) | **11** | 57 (O2: 11) | Scalar mul base fija |
| EscalarMulAny(254) | **2,310** | 2,310 | Scalar mul base variable |
| EdDSAPoseidon | **3,965** | 8,086 | Verificación de firma completa |
| Pedersen(8) | **13** | 89 (O2: 13) | Hash EC, Window4 + Montgomery |
| Mux3 / Mux4 | 16 / 37 | — | Selector 8-way / 16-way |
| Switcher | — | — | Swap condicional |
...
**O2 — Deducción algebraica** (pase opcional con rollback):
5. **DEDUCE**: eliminación gaussiana sobre la matriz de monomios cuadráticos. Encuentra combinaciones lineales de restricciones cuadráticas que producen ecuaciones puramente lineales, introduciendo nuevas ecuaciones de wire sin agregar restricciones. Se aplica iterativamente hasta convergencia.
6. **Propagación de constantes**: sustituye wires probados iguales a constantes de campo, colapsando sub-expresiones enteras. Las constantes cableadas desde inlining de componentes se propagan a través de aritmética Montgomery/Edwards — toda la cadena se reduce a cero restricciones por operación de punto base.
7. **Rollback**: si DEDUCE + propagación constante no reduce el conteo, el pase O2 se descarta y se usa el output de O1.

Resultados en circuitos representativos (pre-opt → O1 → O2):

| Circuito | pre-opt | O1 | O2 | circom O1 |
|---|---|---|---|---|
| Num2Bits(8) | 25 | **9** | — | 17 |
...
Ve [Módulos](/docs/language/modules/) y [Importando Templates de Circom](/docs/circom/importing/) para la referencia completa de sintaxis.

### Nuevo: descomposición del workspace — 19 crates

El código pasó de 10 a 19 crates con un layering coherente:

- **Capa de ejecución** — `akron` (VM de bytecode, renombrado de `vm`), `artik` (VM de testigos), `lysis` (tercera VM, ver abajo).
- **Pipeline de lowering** — `ir-core` (hoja SSA), `ir-forge` (templates ProveIR + lift de Lysis), `zkc` (compilador de constraints — R1CS, Plonkish, witness, oracle), `akronc` (compilador de bytecode, renombrado de `compiler`).
- **Frontends** — `achronyme-parser`, `circom`, `resolve` (nuevo grafo de módulos + resolución de símbolos).
- **Compartidos** — `memory`, `diagnostics`, `lysis-types`, `ach-macros`, `proving`, `cli`, `achronyme-std`.

`compiler` → `akronc`, `vm` → `akron`, `witness` → `artik`. Re-exports mantienen las APIs públicas estables para embedders. El split deja legible el DAG de dependencias: `lysis` solo depende de `{memory, artik, lysis-types}`, el templating de ProveIR ya no arrastra generación de constraints, y el compilador de constraints puede evolucionar sin tocar el IR core.

### Nuevo: Lysis VM — tercera modalidad de ejecución para programas con forma circom

Lysis es una VM basada en registros que ejecuta cuerpos de funciones circom liftados y cuerpos de loops estructuralmente uniformes *como una traza VM*, en lugar de depender del walker para emitir IR por iteración. Esto elimina una clase de bloqueos por desbordamiento de frame que estaban gateando circuitos pesados como SHA-256(64).

- **36 opcodes** incluyendo `EmitDiv`, `EmitAssertEqMsg` y tres variantes con heap (`StoreHeap`, `LoadHeap`, `EmitWitnessCallHeap`).
- **Heap de spill** con la invariante *single-static-store*: cada slot del heap se escribe exactamente una vez antes de cualquier lectura, y las lecturas caen en la misma región dominador que el store. Validado por un pase de integridad; las violaciones se rechazan en tiempo de lift, no en runtime.
- **Header v2** con `heap_size_hint` para que el runtime preallocate la arena de spill antes de iniciar la ejecución.
- **Splits por iteración intermedia** — cuando el cuerpo de una sola iteración excede el presupuesto del frame rolled, el lifter divide a la mitad de la iteración en puntos seguros en lugar de forzar todo el loop a unrolling per-iter. Esto es lo que destraba SHA-256(64).
- **Profundidad de llamada por defecto** subida de 64 a 8192 para acomodar árboles de inlining de templates profundos.

### Nuevo: Artik VM — cómputo de testigos exclusivo

Artik es la VM del lado del testigo. A diferencia de Akron (que corre programas Achronyme completos con heap + GC), Artik no tiene heap, no tiene GC, ~25 opcodes, y se despacha desde R1CS vía `WitnessOp::ArtikCall`. Esto cierra el gap (anteriormente `E212`) donde los cuerpos de funciones circom que necesitaban evaluación de testigo en runtime tenían que recurrir a heurísticas del intérprete — ahora compilan a bytecode `.artik` y corren como datos.

### Nuevo: crate `resolve` — grafo de módulos + resolución de símbolos

Una capa de resolución dedicada que construye un grafo de módulos, anota el AST del programa y expone un `BuiltinRegistry` poblado con los 21 builtins. Adoptado tanto por el compilador de bytecode como por ProveIR — ya no cargan lógica de resolución de nombres en paralelo. Nuevas formas de diagnóstico: `ProveBlockUnsupportedShape`, `FnAlias`. El operador `::` de este release es la superficie visible al usuario de este trabajo.

### Nuevo: dispatch Circom en modo VM

Un nuevo opcode `CallCircomTemplate` (162) y el value tag `TAG_CIRCOM_HANDLE` permiten que programas `.ach` corriendo bajo `ach run` invoquen templates circom importados como valores:
...
Soporta inputs escalares y de array. Los templates compilan a través del mismo frontend circom usado por bloques `prove {}`; luego un `CircomLibraryHandle` mantiene el cuerpo compilado para instanciación en runtime. Ve [Modo VM de Circom](/docs/circom/vm-mode/) para la semántica de llamada y las restricciones actuales.

### Nuevo: sección `[circom]` en `achronyme.toml`

Configura los paths de búsqueda de librerías circom a nivel proyecto:
...
Resuelto relativo a la raíz del proyecto y mergeado con los flags `-l/--lib` de `ach circom`. Se propaga a `ach run` y `ach circuit` para que los bloques prove que importan `.circom` tomen los mismos directorios de librería.

### Perf: optimizador R1CS — O1 iterativo con DEDUCE sparse y clustering

El pase O1 ahora se estructura alrededor de iteración y clustering en lugar de un escaneo de una sola pasada:

- **Eliminación lineal Cluster-Gauss** — las constraints se particionan por conectividad de signals y se eliminan por Gauss cluster por cluster, con un picker condicional al tamaño (tamaño ≤ 500: picker de mínima ocurrencia; arriba: greedy fallback). Working sets más pequeños por paso de Gauss, mayor reducción total.
- **Primitivas in-place** — `simplify_in_place`, `apply_substitution_in_place`. Reduce la presión de allocación del optimizador: runtime completo del benchmark **9.3 s → 6.8 s (−26.5 %)** sin cambiar el output.
- **DEDUCE sparse** — la eliminación gaussiana sobre la matriz de monomios cuadráticos ahora es sparse, con materialización fila por fila. Cierra ~25 % del gap residual de O2 vs el optimizador circom de referencia.
- **Heurística de frecuencia, eliminación de lineales tautológicos, manejo de zero-product** — todo preservado del pase O1 inicial.

Impacto concreto: SHA-256(64) post-O1 = 30,113 constraints (≈ circom O2 ± 4 %); LessThan(8) post-O1 = 9 (vs circom O2 = 20); MiMCSponge(2, 220, 1) post-O1 = 1,317 (vs circom O2 = 1,320).

### Nuevo: inferencia de bit-width y plumbing de `Capture(nBits)`

Un nuevo pase IR infiere bounds de bit-width para shifts y descomposiciones, y `Capture(nBits)` se cablea a través de `InferenceCtx::param_values` para que templates parametrizados sobre bit width (Num2Bits, RangeProof, ShR, ops de palabras de SHA-256) narrowen correctamente sin anotaciones manuales. Las operaciones de shift de SHA-256 narrowean a exactamente 10,473 / 10,473 (sin sobre-descomposición).

### Nuevo: SHA-256(64) end-to-end

El circuito SHA-256 completo compila end-to-end a través del frontend circom:

- **Compile** — 1,287 splits intermedios, 207 k instrucciones IR, 50 ms de tiempo de build de Lysis.
- **Witness** — handler de `LetArray` en `compute_witness_hints_with_captures` materializa los aliases `var outCalc[256] = sha256compression(...)`. El digest es **bit-exacto** versus `sha2::Sha256::digest` (FIPS-180-4) sobre los vectores de prueba elegidos.
- **R1CS** — 87,391 constraints en hard-gate; 30,113 post-O1 (dentro del 4 % del output de referencia O2 de circom).

### Perf: fix cuadrático en `do_split` del walker

La lógica de split intermedio del walker re-escaneaba el body por cada split (O(n²) en cantidad de iteraciones). Un `last_use_idx: HashMap<SsaVar, usize>` precomputado reemplaza el rescan por split:

- SHA-256(64) instantiate: **7.85 s → 1.87 s (−76 %)**.
- SHA-256(64) compile total: **20.57 s → 14.65 s (−28.8 %)**.
- Output R1CS: byte-idéntico (8/8 benchmark + 3/3 cross-mode adversariales).

### Nuevo: suite de soundness adversarial

Evidencia empírica de que los conteos de constraints bajo el baseline O2 de circom no son atajos under-constrained:

- **Num2Bits** — forjar un bit no booleano, el chequeo de witness rechaza.
- **Num2Bits sum violation** — forjar un patrón de bits cuya suma no concuerda con el input.
- **LessThan(8)** — forjar el output del comparador en ambas direcciones; ambas falsificaciones rechazadas por el sistema de constraints.

Cada reducción R1CS en la tabla del benchmark está apareada con un test adversarial confirmando que la invariante de soundness sobrevive a la optimización. Esta es la traza de auditoría para "Achronyme O1 ≤ circom O2 — pero sigue siendo sound".

### Nuevo: guardas de profundidad del parser (E300)

El parser Circom ahora capa la recursión de expresión y bloque a 64 niveles, devolviendo `E300` en lugar de hacer stack-overflow ante input adversarial. Surfaceado por el soak de libFuzzer — dos crashes distintos (`[` profundamente anidado, `{` profundamente anidado) ambos arreglados bajo la misma forma de diagnóstico, con tests de regresión para cada uno.

### Hardening: soak de fuzz de 10M iteraciones

Siete targets de fuzz (`fuzz_lysis_decode`, `fuzz_lysis_validate`, `fuzz_artik_decode`, `fuzz_proveir_instantiate`, `fuzz_parser`, `fuzz_circom_parser`, `fuzz_r1cs_preservation`) corren limpios por 10M iteraciones por target. Tres targets restantes (`fuzz_ir_lower`, `fuzz_artik_exec`, `fuzz_r1cs_pipeline`) golpean la política `rss_limit_mb` de libFuzzer con inputs propensos a amplificación; la invariante `no panics` del target se mantiene, y las clases de OOM están trackeadas como follow-ups post-tag.

### Nuevo: validador de integridad SSA por pase

Un validador opt-in (`ACHRONYME_VALIDATE_IR_PASSES=1`) corre tras cada pase IR y atrapa violaciones de invariantes SSA — definiciones duplicadas, usos colgados, mismatches de dominator-frontier — que de otro modo aparecerían como miscompiles silenciosos. Off por default; actívalo cuando estés triajeando miscompiles o desarrollando pases nuevos.

### Mejorado: EdDSAPoseidon end-to-end

Compile + instantiate + verificación R1CS todo en verde: **31,685 nodos IR → 22,271 instrucciones** tras optimización, **3,965 constraints post-optimizador** (2.04× menos que circom `--O1` con 8,086). El test R1CS pasa con `enabled=0`, encima del test Groth16 existente.
