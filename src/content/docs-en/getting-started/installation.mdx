...
The currently published stable version is `0.0.1`. The source tree and web
package are already versioned `0.1.0`, but that candidate is not returned by
the installer until its tag and release artifacts are published. Pin the
stable version explicitly when reproducibility matters:
...
## Evaluate the 0.1.0 candidate from source

Achronyme is written in Rust. The candidate release toolchain is Rust 1.96.0.
Use the exact candidate revision when reviewing 0.1.0 behavior:
...
backends. In the 0.1.0 candidate, proof generation fails unless you choose
