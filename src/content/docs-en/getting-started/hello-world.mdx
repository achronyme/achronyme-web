---
title: Hello World
description: Your first Achronyme program and your first circuit.
---

## Create a project

The fastest way to get started is with `ach init`:

```bash
ach init hello --template vm
cd hello
ach run
```

This creates a project with an `achronyme.toml` and `src/main.ach`. See [Project Configuration](/docs/cli/project-config) for details.

You can also create files manually — no project setup required.

## General-Purpose Program

Create a file `hello.ach`:

```ach
print("Hello, world!")
```

Run it:

```bash
ach run hello.ach
```

Output:

```ach
Hello, world!
```

### A more interesting example

```ach
let fib = fn fib(n) {
    if n < 2 { return n }
    return fib(n - 1) + fib(n - 2)
}

for i in 0..10 {
    print(fib(i))
}
```

## Your First Circuit

Create a file `multiply.ach`:

```ach
circuit multiply(product: Public, a: Witness, b: Witness) {
    assert_eq(a * b, product)
}
```

This circuit proves that the prover knows two secret numbers `a` and `b` whose product equals the public value `product`.

Compile it:

```bash
ach circuit multiply.ach --inputs "product=42,a=6,b=7"
```

This produces `circuit.r1cs` and `witness.wtns`, ready for snarkjs proof generation.

### What just happened?

1. The parser read `multiply.ach` and built an AST with the `multiply` circuit definition
2. The IR lowering phase converted it to SSA instructions
3. The R1CS backend compiled the multiplication into a single constraint: `a * b = product`
4. The witness generator filled in the concrete values: `a=6`, `b=7`, `product=42`
5. The exporter wrote snarkjs-compatible binary files

## Inline Proof

You can also generate proofs directly from Achronyme code:

```ach
let a = 0p6
let b = 0p7
let product = 0p42

let p = prove(product: Public) {
    assert_eq(a * b, product)
}
```

The `prove {}` block compiles the circuit, captures variables from the enclosing
scope, generates a witness, verifies the constraints, and returns a real
Groth16 proof. Achronyme 0.1.0 deliberately does **not** choose the proving
trust model for you.

For local development, opt in to single-party setup explicitly:

```bash
ach --insecure-dev-setup run multiply.ach
```

The flag makes iteration convenient, but its keys and proofs are not production
trusted. Production proving selects a ceremony-derived store bound to the
exact circuit:

```bash
ach --trusted-key-dir ./trusted-keys run multiply.ach
```

Without either option, proof generation fails closed. Continue with
[Proof Generation](/docs/zk-concepts/proof-generation/) before using proof
artifacts outside local development.
