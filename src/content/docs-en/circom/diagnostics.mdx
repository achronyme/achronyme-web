...
| `use P::Template instead of P.Template` | Migration error: dot notation on a module or circom alias is no longer accepted — use the `::` path operator. |
...
| `template argument must be an integer literal` | VM-mode call constraint (see [VM Mode Limitations](/docs/circom/limitations/#vm-mode-limitations)). |
