...
VM mode was introduced with the circom frontend and is intentionally narrower in scope than [Circuit Mode](/docs/circom/circuit-mode/).
...
Template arguments follow the same rule as circuit mode, with one additional VM-mode restriction:

- **Must be compile-time constants** (same as circuit mode).
- **Must be integer literals** at the call site.
...
- **Array signal inputs** — calling a template that takes `signal input in[n]`. This is a VM-mode-only restriction tied to the `CallCircomTemplate` opcode's register layout, not a witness-evaluation limit.
...
These are scoping limits of VM-mode *dispatch*. The underlying witness evaluator itself is fully general — circuit-mode witness generation already handles array template parameters and multi-limb bignum routines (for example, secp256k1 ECDSA witnesses run through the Artik VM). All of these are tracked in [Limitations and Roadmap](/docs/circom/limitations/). None of them affect circuit-mode dispatch, which remains the primary supported path.
