...
For project-wide configuration, declare the same directories in `[circom].libs`. `ach run`, `ach circuit`, and `ach circom` all consume that list; command-line `-l/--lib` entries append extra directories for a single invocation.
...
The `[circom]` section in `achronyme.toml` declares per-project library search paths:
...
`ach run`, `ach circuit`, and `ach circom` consume `[circom].libs` automatically, matching the ergonomics of `[project].entry` and the other manifest-driven defaults. CLI `-l/--lib` flags append to that list rather than replacing it. See [Limitations and Roadmap](/docs/circom/limitations/) for follow-up work in this area.
