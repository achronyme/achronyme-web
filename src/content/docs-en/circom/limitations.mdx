...
| `EdDSAPoseidonVerifier` | — | — | ✓ Compile + instantiate (31,685 IR nodes → 22,271 instructions after optimisation); R1CS test passes with `enabled=0` |
| `ECDSAVerify(64, 4)` / secp256k1 ECDSA | — | — | ✓ Compiles + generates a witness + proves end-to-end (Groth16); ~1.49M optimized R1CS constraints, below circom for the same circuit; witness verifies |
| Mux, gates, bitify, comparators | — | — | ✓ Used transitively by the above |

`ECDSAVerify(64, 4)` is the heaviest circomlib reference circuit, and its witness depends on secp256k1 multi-limb arithmetic — modular inverse and exponentiation, schoolbook product, and long division. Achronyme now generates that witness natively through Artik VM intrinsics, instead of an interpreted Fermat modular-inverse path, so the full circuit compiles, witnesses, and proves without manual intervention.
...
### Structural — by design (circuit mode)

These restrictions come from Achronyme's **circuit-mode** lowering model (fully-unrolled IR, R1CS-first), not from missing work. They apply to constraint generation; witness computation is handled separately by the Artik VM, which is more permissive (see the note below):
...
The unrolling restriction is specific to the R1CS/Plonkish constraint IR. **Witness functions lifted into the Artik VM do support runtime-bound loops and variable-index access** — that is exactly how secp256k1 ECDSA witness generation works, where multi-limb bignum routines (modular inverse, long division) run with loop bounds and array indices that are only known at runtime. The constraint system stays fully unrolled; the witness side does not need to be.
...
VM mode (calling templates from non-`prove` code under `ach run`) is narrower than circuit mode. Current restrictions:
...
### Tutorials and migration guides (in progress)
