...
import { crateMapSvgEn } from '../../../lib/diagrams/crate-map';
...
| Crate | Path | Purpose |
|-------|------|---------|
| `diagnostics` | `diagnostics/` | Spans, errors, suggestions, rustc-style renderer (zero-dep leaf) |
...
| `achronyme-parser` | `achronyme-parser/` | Lexer, Pratt+recursive-descent parser, owned AST types |
| `resolve` | `resolve/` | Symbol table, builtin registry, unified dispatch resolver (Movimiento 2) |
| `lysis-types` | `lysis-types/` | `InstructionKind<F>`, `NodeId` shared with the [Lysis VM](/docs/architecture/lysis-vm/) |
| `constraints` | `constraints/` | `Variable`, `LinearCombination`, R1CS + Plonkish system, Poseidon, binary export |
| `ir-core` | `ir-core/` | SSA leaf: `Instruction<F>`, `SsaVar`, `Visibility`, `IrType` |
| `ir-forge` | `ir-forge/` | ProveIR templates, AST→ProveIR compiler, Lysis lift walker (lifts ProveIR to BOC bytecode for the [Lysis VM](/docs/architecture/lysis-vm/)) |
| `ir` | `ir/` | IR passes (DCE, CSE, const-fold, taint), module loader, inspector, ProveIR orchestration |
| `zkc` | `zkc/` | Constraint compiler: R1CS backend, Plonkish backend, witness ops, oracle scaffold |
| `akronc` | `akronc/` | Bytecode compiler for the [Akron VM](/docs/architecture/akron-vm/) (was `compiler`) |
| `akron` | `akron/` | Register-based general-purpose bytecode VM (was `vm`) |
| `artik` | `artik/` | Witness-computation SSA VM (was `witness`) — see [Artik](/docs/architecture/artik-vm/) |
| `lysis` | `lysis/` | Third VM, [BOC](/docs/architecture/lysis-vm/#bytecode-oriented-compilation-boc) executor: runs lifted bytecode and emits SSA IR via the interning sink — **in progress** |
| `circom` | `circom/` | Circom 2.x frontend: lexer, parser, analysis, lowering — see [Circom Frontend](/docs/architecture/circom-frontend/) |
| `proving` | `proving/` | Groth16 (arkworks), PlonK (halo2-KZG), Solidity verifier generation |
| `achronyme-std` | `std/` | Standard library via `NativeModule` trait |
...
ProveIR data types (`ProveIR`, `CircuitExpr`, `CircuitNode`, `CaptureDef`), AST→ProveIR compiler (`ProveIrCompiler`), Lysis lift walker (the AST→Bytecode half of the [BOC](/docs/architecture/lysis-vm/#bytecode-oriented-compilation-boc) pipeline). The single layer where compile-time symbolic circuit reasoning happens. See [ProveIR](/docs/architecture/prove-ir/).
...
Register-based general-purpose VM. 34 assigned opcode variants, bitmap mark-and-sweep GC, 64-bit tagged values. Loads `.achb` files via `akron::loader`. Dispatches `prove {}` execution at runtime via the injected `ProveHandler` trait.
...
Third VM — **in progress**. [BOC](/docs/architecture/lysis-vm/#bytecode-oriented-compilation-boc) executor: structural template instantiator with hash-consing interner that runs lifted bytecode and emits SSA IR via the interning sink. See [Lysis VM](/docs/architecture/lysis-vm/). Walker lives in `ir-forge::lysis_lift`; oracle scaffold in `zkc::lysis_oracle`.
...
<AchDiagram svg={crateMapSvgEn} ariaLabel="Achronyme crate dependency graph" />
...
| Instantiate ProveIR | `ir-forge` | `ProveIR::instantiate_lysis(captures)` / `instantiate_lysis_with_outputs(...)` |
