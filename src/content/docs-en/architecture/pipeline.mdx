...
import { compilePipelineSvg } from '../../../lib/diagrams/compile-pipeline';
...
<AchDiagram svg={compilePipelineSvg} ariaLabel="Achronyme compile pipeline" />
...
For `ach run`, the AST is compiled to bytecode for the register-based VM (34 assigned opcode variants). A 3-pass bytecode optimizer runs after compilation:
...
| D | Instantiate | ProveIR + captured scope values → SSA IR (Bytecode-Oriented: lifted to a [Lysis VM](/docs/architecture/lysis-vm/) program whose execution emits SSA) |
...
Compiles `.circom` files through the same ProveIR → IR → Backend pipeline. The Circom frontend adds structural safety: a `<--` / `-->` witness hint that is not pinned by a corresponding constraint is reported as E100. Double assignments are reported separately as W103 warnings, so the docs should not treat W103 as a constraint.
...
| Symbol resolution | `resolve/src/annotate/` |
| Circom Lexer | `circom/src/lexer/` |
| Circom Parser | `circom/src/parser/` |
| Circom Analysis | `circom/src/analysis/` |
| Circom Lowering | `circom/src/lowering/` |
| Bytecode Compiler | `akronc/src/codegen/` |
| Bytecode Optimizer | `akronc/src/optimizer/` |
| ProveIR Compiler | `ir-forge/src/ast_lower/api.rs` |
| ProveIR Instantiation | `ir-forge/src/instantiate/` |
| Lysis Lift | `ir-forge/src/lysis_lift/` |
...
| Witness Generation | `zkc/src/witness/` |
