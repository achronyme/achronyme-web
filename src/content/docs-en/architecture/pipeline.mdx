...
Achronyme compiles the same source language through three distinct paths: **VM mode** for general execution, **Circuit mode** for constraint generation via `ach circuit`, and **Prove mode** for inline proof generation via `prove {}` blocks.
