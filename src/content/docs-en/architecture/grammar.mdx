...
- File: `achronyme-parser/src/lexer/`
...
The full `TokenKind` enum lives in `achronyme-parser/src/token.rs`. Below it is grouped by role. A token is `(kind, span, lexeme)`; `lexeme` is borrowed from the source for cheap identifier comparison.
...
Pratt parser. Source: `achronyme-parser/src/parser/tables.rs`. The Pratt loop reads `left_bp` and `right_bp` from `infix_binding_power(token)` and recurses while `right_bp >= caller_bp`.
...
Lower level number = **higher** binding power. Exponent is right-associative because in `achronyme-parser/src/parser/tables.rs` its `left_bp > right_bp`, so `a ^ b ^ c` parses as `a ^ (b ^ c)`.
...
For expression errors inside a statement, the parser returns `Expr::Error { id, span }` and lets the statement-level recovery catch the rest. See `achronyme-parser/src/parser/core.rs` for the `synchronize()` helper.
...
| Parser core (Pratt)           | `achronyme-parser/src/parser/core.rs` |
| Expression parsing            | `achronyme-parser/src/parser/exprs.rs` |
| Statement parsing             | `achronyme-parser/src/parser/stmts.rs` |
| Precedence + dispatch tables  | `achronyme-parser/src/parser/tables.rs` |
