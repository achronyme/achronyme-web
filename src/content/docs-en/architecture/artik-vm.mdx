...
- **Side-channel via `Instruction::WitnessCall`, not a separate side-table on `IrProgram`.** Keeps witness programs in instruction order so the prover's witness-gen loop naturally interleaves them between other `WitnessOp` replays. Cost: ~8 exhaustive match sites extended across `ir` + `compiler`.
...
| R1CS dispatch | `compiler/src/witness_gen.rs::dispatch_artik_call` |
