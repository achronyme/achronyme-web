...
- File: `achronyme-parser/src/ast.rs`
- Every node carries a `Span`. Re-exported from `diagnostics/src/span.rs`.
...
From `achronyme-parser/src/ast.rs:52-141`:
...
From `achronyme-parser/src/ast.rs:188-331`. Every variant carries `id: ExprId` and `span: Span` in addition to the listed fields.
...
| Variant | Extra fields | Notes |
|---------|--------------|-------|
| `Error` | — | Emitted by parser sync points (`synchronize()` in `achronyme-parser/src/parser/core.rs`). |
...
- The resolver pass annotates each `ExprId` with a `SymbolId` in `SymbolTable`. See `resolve/src/annotate/`. The resolver does **not** mutate the AST — it builds a side table keyed by `ExprId`.
...
| Component             | File |
|-----------------------|------|
| AST types             | `achronyme-parser/src/ast.rs` |
| Span                  | `diagnostics/src/span.rs` |
| Parser entry          | `achronyme-parser/src/lib.rs` |
| Resolver annotation   | `resolve/src/annotate/` |
| Symbol table          | `resolve/src/table.rs` |
