...
- The resolver pass annotates each `ExprId` with a `SymbolId` in `SymbolTable`. See `resolve/src/annotate.rs`. The resolver does **not** mutate the AST — it builds a side table keyed by `ExprId`.
...
| Resolver annotation   | `resolve/src/annotate.rs` |
