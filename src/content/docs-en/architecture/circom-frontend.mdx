...
File: `circom/src/lexer/`. Single-pass byte scanner. Maximal-munch disambiguation for the signal-operator family:
...
1. **Constraint pairing check** — `<--` / `-->` assignments are witness hints, not constraints. If the assigned signal is not pinned directly by `===` or indirectly through a tracked `var` accumulator, the analyzer emits **E100**. Double assignment is a separate **W103** warning.
...
- `circom/src/lowering/mod.rs` — driver
- `circom/src/lowering/components.rs` — component instantiation, parameter binding
- `circom/src/lowering/signals.rs` — signal dimension extraction
- `circom/src/lowering/expressions/` — `CircuitExpr` lowering
- `circom/src/lowering/statements/` — `CircuitNode` lowering
- `circom/src/lowering/const_fold.rs` — compile-time constant evaluation (BigVal, ternary fold, array constants)
- `circom/src/lowering/artik_lift/` — function bodies that can't inline → Artik bytecode
- `circom/src/lowering/context.rs` — environment, captures, known_constants
...
The benchmark harness records constraint counts against `circom --O2` for reference templates such as Num2Bits, LessThan, MiMCSponge, and EscalarMulAny. Treat exact counts as benchmark output, not architecture: rerun `circom/tests/e2e_optimization/benchmark/` before publishing current numbers.

## Adversarial soundness tests

File: `circom/tests/adversarial.rs`. The adversarial tests cover forged witnesses for representative primitives so optimizer wins do not silently become under-constrained circuits. Examples include:
...
| Expressions | `circom/src/lowering/expressions/` |
| Statements | `circom/src/lowering/statements/` |
| Const fold | `circom/src/lowering/const_fold.rs` |
| Artik lift | `circom/src/lowering/artik_lift/` |
| Context | `circom/src/lowering/context.rs` |
| Diagnostics | `diagnostics/src/` and `circom/src/lib.rs` |
| Adversarial tests | `circom/tests/adversarial.rs` |
| E2E tests | `circom/tests/` |
