...
`WitnessCall` carries an embedded Artik bytecode blob — see [Artik Witness VM](/docs/architecture/artik-vm/). `ComponentCall` points into `ProveIR::component_bodies`; instantiation expands the shared body with the component name and parameter substitutions instead of serializing another inlined copy.
