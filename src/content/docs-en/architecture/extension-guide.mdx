...
In `zkc/src/plonkish_backend.rs`, add a match arm in `compile_ir`:
...
If the instruction allocates intermediate variables, record a `WitnessOp` in `zkc/src/r1cs_witness.rs`.
...
In `akronc/src/codegen.rs`, emit the opcode during compilation:
...
- [ ] `zkc/src/plonkish_backend.rs` — Plonkish gate emission
- [ ] `zkc/src/r1cs_witness.rs` — witness computation (if intermediate wires)
