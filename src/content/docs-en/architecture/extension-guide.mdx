...
In `ir-core/src/types/instruction.rs`, add a new variant to the `Instruction` enum:
...
In `ir/src/lower/`, handle the new instruction in the appropriate method (e.g., `lower_call` for builtins, `lower_expr` for operators):
...
In `ir/src/eval/`, handle the instruction in the appropriate evaluator:
...
In `zkc/src/r1cs_backend.rs`, add a match arm in `compile_ir`:
...
In `zkc/src/plonkish_backend/`, add a match arm in `compile_ir`:
...
If the instruction allocates intermediate variables, record a `WitnessOp` in `zkc/src/witness/`.
...
In `ir/src/lower/`, add a match arm in `lower_call`:
...
Create a pass file under `ir/src/passes/`, for example `my_pass.rs`:
...
In `akron/src/stdlib/core.rs`, add the implementation:
...
In `akron/src/specs.rs`, add an entry to `NATIVE_TABLE`:
...
In `akron/src/machine/native.rs`, add the match arm in `bootstrap_natives`:
...
In `akron/src/opcode.rs`, add a constant:
...
In `akronc/src/codegen/`, emit the opcode during compilation:
...
In `akron/src/machine/vm.rs`, add a match arm in `interpret_inner`:
...
- [ ] `ir-core/src/types/instruction.rs` — `Instruction` enum + `result_var` + `has_side_effects` + `operands`
- [ ] `ir/src/lower/` — emit instruction from AST
- [ ] `ir/src/eval/` — concrete evaluation
...
- [ ] `zkc/src/r1cs_backend.rs` — R1CS constraint generation
- [ ] `zkc/src/plonkish_backend/` — Plonkish gate emission
- [ ] `zkc/src/witness/` — witness computation (if intermediate wires)
