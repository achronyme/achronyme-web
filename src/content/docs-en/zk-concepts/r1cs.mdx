...
Use `--circuit-stats` with `ach run`, `ach circuit`, or `ach circom` to inspect
constraint costs. Achronyme reports two deliberately different totals:

- `PRE-OPTIMIZATION ESTIMATE` is the static estimate derived from optimized IR
  categories before R1CS finalization.
- `FINAL PROVING CONSTRAINTS` is the exact count after R1CS optimization. This
  is the constraint system passed to Groth16 and bound to trusted keys.

Use the final count for proving capacity, ceremony, and release evidence. The
estimate is useful for attribution, but it is not a cryptographic artifact
identity.
