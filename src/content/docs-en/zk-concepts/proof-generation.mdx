...
> **Known limitation:** the Groth16 trusted setup is **development-only**. The proving and verifying keys are generated locally with a CSPRNG (`OsRng`), not produced by a multi-party ceremony. For a production deployment you need a real ceremony — ingesting external Powers of Tau is the planned path and is not yet wired.
