...
### Expanded: circomlib E2E R1CS coverage — 13 verified circuits

Full constraint system verification (compile → instantiate → R1CS → witness → verify) now covers 13 real circomlib circuits:

| Circuit | Constraints | Pattern |
|---|---|---|
| Num2Bits(8) | 17 | Bit decomposition |
| IsZero | 4 | Inverse gadget |
| LessThan(8) | ~20 | Component composition |
| Poseidon(2) | ~361 | Permutation + MDS |
| MiMCSponge(2,220,1) | ~3,000 | Feistel, 220 rounds |
| BabyJubjub (Add+Dbl+Check) | 48 | Edwards curve |
| EscalarMulFix(3) | — | Montgomery + Mux3 |
| EscalarMulAny(254) | 10,224 | Full scalar mul (Groth16 verified) |
| **EdDSAPoseidon** | **>10,000** | **Full signature verification** |
| **Switcher** | — | Conditional swap |
| **Mux3 / Mux4** | — | 8-way / 16-way selector |
| **AliasCheck** | 201,267 | Field modulus alias detection |
| **Sign** | 1,181 | Field element sign |
| **Pedersen(8)** | **394** | **EC hash (Window4 + Montgomery)** |

EdDSaPoseidon (the "boss final" — full EdDSA signature verification over BabyJubjub with Poseidon hash) and Pedersen (elliptic-curve-based hash, completely different construction from algebraic hashes) represent the two most complex circomlib circuits successfully verified.

### New: R1CS linear constraint optimizer

Multi-pass R1CS optimizer that reduces constraint count after compilation:

1. **Linear elimination**: substitutes wires defined by linear constraints (`k * LC = LC`), removing the constraint entirely
2. **Frequency heuristic**: selects the most-referenced wire for substitution to maximize cascading simplifications
...
EdDSaPoseidon: 8,514 → 7,752 constraints (0.96× vs circom `--O1` at 8,086).
