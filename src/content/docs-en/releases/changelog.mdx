...
## 0.1.2 - detached verification patch

> Published on 2026-08-09 from core revision
> `cd7a6e66e133bebd8e2026e321a4c85023c311f7`.

**Achronyme 0.1.2 corrects detached verification output without changing the
language, bytecode, runtime ABI, proof formats, trusted-key format, or LSP
contracts.**

- Missing, unreadable, or uninspectable detached artifacts report their
  operational cause instead of the misleading `proof invalid` result.
- Global `--error-format json` selects structured detached-verification output
  unless an explicit `verify --format` overrides it.
- Legacy strategy and audit records were removed from the current repository
  tree without rewriting historical commits or tags.

---

## 0.1.1 - maturity patch

> Published on 2026-08-08 from core revision
> `b1774e88671a1889146804cb812cb099eb9cc006`.

**Achronyme 0.1.1 hardens the contracts introduced in 0.1.0 without changing
the bytecode, runtime ABI, trusted-key format, or proving policy.**

### Correctness and observability

- Imported-module diagnostics retain the canonical file, exact span, and
  matching source in human, JSON, and short output.
- Namespace and selective imports preserve their defining module context
  inside proof compilation, including typed array captures.
- Circuit statistics separate the pre-R1CS-optimization estimate from the
  exact finalized constraint count used for proving.
- `max_tasks` counts every live child task, while `max_task_scopes` counts all
  simultaneously live structured scopes across the runtime.

### End-to-end maturity fixture

- Tilino Lab is a modular private-auction program covering namespace modules,
  typed arrays, structured concurrency, bounded channels, owned TCP and file
  resources, explicit capabilities, Poseidon commitments, Merkle membership,
  and a Groth16 winner proof.
- Its contract checks interpreter/JIT parity, detached verification, tampered
  public inputs, denied authority, resource exhaustion, exact R1CS statistics,
  and the standalone AOT boundary.
...
> Published on 2026-08-08 from core revision
> `fd07b38e16256e2ed6a8f2b438d340a681c9b0ac` after the independent
> ceremony, public beacon, and bidirectional interoperability gate completed.

**Achronyme 0.1.0 adds structured concurrency, explicit host authority, and a
fail-closed proving trust boundary.**

### Structured concurrency and host authority

- Lexical `concurrent` scopes add scope-bound `spawn`, explicit `await`, task
  races, cooperative cancellation, bounded channels, and task diagnostics.
- Effect inference and capability manifests keep host execution separate from
  `prove` and `circuit` code.
- File and TCP resources have explicit ownership, bounded execution, and
  deterministic cleanup. Filesystem and network access require exact grants.
- The interpreter, LLVM JIT, AOT runtime, and WASM support matrix define the
  same structured task semantics without granting ambient host access.

### Proving trust

- Proof generation denies implicit local setup. Development proofs require an
  explicit insecure opt-in.
- Production BN254 Groth16 proving can load an immutable trusted-key store
  bound to the exact optimized R1CS and ceremony transcript.
- Detached BN254 and BLS12-381 Groth16 verification is available from the CLI.
- BLS12-381 Groth16 and Plonkish proof generation remain development-only in
  0.1.0.
- The web playground selects its development setup policy centrally and labels
  its proof artifacts as not production trusted.

### Compatibility and verification

- `concurrent`, `spawn`, and `await` are now reserved words. The runtime still
  loads 0.0.1 bytecode while 0.1.0 emits the new structured-task format.
- Release CI covers the full workspace, proving interoperability, and LLVM JIT
  and AOT gates on Linux x86_64 and aarch64.
- The production ceremony records the independent phase-2 contribution, a
  precommitted public beacon, and bidirectional snarkjs interoperability.

---

## 0.0.1 - 2026-08-02

**Achronyme starts plain `MAJOR.MINOR.PATCH` numbering for its pre-1.0 line.**
This intentional numbering reset follows the historical `0.1.0-beta.22`
series. The language and API can still change between `0.x` minor versions.

### Native execution and packaging

- LLVM 21 ORC JIT is part of the default CLI, with explicit interpreter
  selection and a visible fallback when LLVM is unavailable.
- `ach aot` emits native executables and discovers the packaged Achronyme AOT
  runtime without manual environment variables.
- Linux releases include reproducible bundles, license files, SHA-256
  checksums, and legacy standalone binaries for existing installers.
- The web installer verifies the Linux bundle and installs both `ach` and the
  AOT runtime.
- `ach-server` pins all compiler and proving dependencies to the same `0.0.1`
  source revision.

### Verification

- The production gate passes on native Linux x86_64 and aarch64 runners.
- The gate covers JIT, interpreter fallback, AOT, resource limits, real file
  I/O, installed-layout execution, and lazy LLVM loading.
- macOS and Windows remain interpreter-compatible release targets; they are not
  certified by the Linux native gate.

---

## 0.1.0-beta.22 — 2026-06-12

**secp256k1 `ECDSAVerify(64, 4)` — the heaviest circomlib reference circuit — now compiles, generates a witness, and proves end-to-end.** The optimized R1CS lands at ~1.49M constraints, below circom's own count for the same circuit; the witness verifies and a Groth16 proof is produced and checked.

### New: native Artik bignum intrinsics

ECDSA witness generation used to lean on an interpreted Fermat modular-inverse path. The Artik VM now ships native bignum intrinsics — `ModInv`, `ModExp`, `Prod`, and `LongDiv` — that replace the interpreted inner loop. The `ECDSAVerify(64, 4)` witness walk drops from ~226s to ~64s end-to-end with the constraint count and verification result unchanged.

### New: multi-subprogram Artik VM

The Artik builder now emits multi-subprogram programs with cross-frame `Call`/`Return`, 2D row slices, descending and runtime-bound loops, array parameters, and field-precision bigint ops. A field-level unsigned ordered compare (`FCmpLt`) was added so signed/unsigned comparisons stay at full field precision. New opcodes since the previous docs: `Call`, `FIDiv`, `FIRem`, `FShr`, `FAnd`, `FPow2`, `FCmpLt`, `ArrayId`, `ArrayFromId`.

### Security: R1CS substitution soundness fix

The greedy R1CS substitution path could leave eliminated wires still referenced after optimization — a forgeable-witness hazard. This is fully closed via per-cluster cycle resolution: witnesses stay bit-identical and the optimized constraint count holds below circom. This is a security-relevant correctness fix, not a performance change.

### Performance

- **Instantiate fusion**, **witness-environment slot replay**, and a **fused optimizer over interner events** cut redundant per-instance work along the lean prove path.
- **Compile-axis amortization** plus a **jemalloc** allocator reduce fragmentation and repeated walks.
- Peak resident memory on ECDSA-scale circuits is roughly halved.

### New: benchmark coverage

The benchmark corpus adds the **Semaphore** main circuit and more circomlib templates alongside the ECDSA reference.

### Tooling

- CI now runs on a **self-hosted runner with a pinned toolchain**, eliminating clippy drift between releases.
...
- `ECDSAVerify(64, 4)` compiles, generates a verifying witness, and produces a Groth16 proof that verifies on BN254.
- Optimized R1CS ~1.49M constraints, below circom for the same circuit.
- Workspace test gate 4,400+ passing across 19 crates.
- Witness output bit-identical before and after the substitution soundness fix.
