...
Values are parsed as field elements in the active prime field (BN254 by default, configurable via `--prime`). Supports decimal, hex (`0xFF`), and negative values. The two flags are mutually exclusive.
...
| `--solidity <path>` | Write a Solidity verifier contract to `<path>` (R1CS/Groth16 only). The path is required. |
