...
When a diagnostic originates in a module, every output format retains the
canonical imported file, exact span, and matching imported source text. An
error in `lib/proofs.ach` therefore never renders against the root program's
line or snippet.
...
This warning only applies to selective imports (`import { ... } from`). Namespace imports (`import "..." as alias`) are not checked — every access goes through the `::` path operator (`alias::name`), so usage is tracked per-member by the compile-time dispatch lookup rather than by a per-alias flag.
