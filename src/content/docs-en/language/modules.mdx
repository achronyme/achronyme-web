...
Access exported values using the `::` path operator: `alias::name`. The lookup is fully resolved at compile time — there is no runtime `alias` object, and the mangled global `alias::name` goes straight through the same dispatch table used by built-in statics like `Int::MAX` or `Field::ZERO`.

:::note
Previous releases used dot notation (`alias.name`). That form now produces a migration error pointing at the correct `alias::name` spelling. The change was made so module-qualified calls resolve identically inside VM code and inside `prove {}` blocks, where the old runtime-map dispatch was not available.
:::
...
print(math::mul(3, 4))   // namespace — with `::`
```
...
Namespace and selective imports preserve the defining module's resolver
context inside `prove` blocks. Typed array captures can cross from one module
into an imported proving function without colliding with expression IDs from
the root file.
...
| `alias::name` | Access a namespace binding (compile-time resolved) |
