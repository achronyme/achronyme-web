...
| `ProveHandlerNotConfigured` | `prove {}` reached but no prove handler was injected into the VM. This only happens in custom embeddings that run the VM directly — `ach run` always wires the native Groth16 handler, so the CLI never triggers it. |
