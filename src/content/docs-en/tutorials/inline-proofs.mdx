...
`ach run` generates real Groth16 proofs natively — no Powers of Tau file or extra flags required:

```bash
ach run my_program.ach
```

The native backend is wired automatically, so every `prove {}` block compiles its circuit, verifies the constraints, and returns a cryptographic proof. The proving and verifying keys are cached in `~/.achronyme/cache/` for faster subsequent runs.

> **Note:** the Groth16 trusted setup is development-only — keys are generated locally with a CSPRNG, not a production ceremony. Ingesting external Powers of Tau for a production-grade setup is the planned path and is not yet wired.
