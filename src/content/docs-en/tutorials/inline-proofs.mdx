...
`ach run` generates proofs only after you select an explicit key source. Use
single-party setup for local development:

```bash
ach --insecure-dev-setup run my_program.ach
```

Every `prove {}` block compiles its circuit, verifies the witness, and returns
a cryptographic proof. Development keys are cached for faster subsequent runs,
but they are not production trusted.

For production BN254 Groth16, select a ceremony-derived store whose manifest
matches the exact optimized circuit:

```bash
ach --trusted-key-dir ./trusted-keys run my_program.ach
```

Without either option, generation fails closed. See
[Proof Generation and Trust](/docs/zk-concepts/proof-generation/) before using
the result outside local development.
...
Plonkish proof generation remains development-only in the 0.1.0 release line.
